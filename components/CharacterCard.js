...
const CharacterCard = memo(function CharacterCard({ character, historicalData = null }) {
    ...
    >
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { xs: 'center', sm: 'flex-start' }, mb: 3 }}>
          ...
          <Box sx={{ textAlign: { xs: 'center', sm: 'left' } }}>
            ...
            <Typography variant="body2" color="text.secondary" aria-label={`伺服器: ${character.world_name}`}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
          ...
          <Typography variant="body2" aria-label={`等級 ${character.character_level}`}>
        ...
        {character.character_date_create && (
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
            ...
            <Typography variant="body2">
              {new Date(character.character_date_create).toLocaleDateString('zh-TW')}
            </Typography>
          </Box>
        )}
      ...
      <Box sx={{ mb: 3 }} aria-labelledby="progress-section">
        ...
        <ProgressBar progress={progress} expRate={5} historicalData={historicalData} />
      </Box>
      ...
      <Typography
        ...
        dateTime={character.date ? new Date(character.date).toISOString() : new Date().toISOString()}
        aria-label={`最後更新時間 ${character.date ? new Date(character.date).toLocaleString() : new Date().toLocaleString()}`}
      >
        最後更新: {character.date ? new Date(character.date).toLocaleString() : new Date().toLocaleString()}
      </Typography>
    </CardContent>
  )
})
