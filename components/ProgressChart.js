...
const ProgressChart = memo(function ProgressChart({ progressData }) {
  ...
  const chartData = useMemo(() => {
    ...
    if (validData.length >= 2) {
      ...
      if (sortedData.length >= 2) {
        ...
        const daysDiff = Math.max(1, (new Date(lastData.date) - new Date(firstData.date)) / (1000 * 60 * 60 * 24))
  ...
  if (chartData.type === 'pie') {
    return (
      <figure className="w-full h-32 sm:h-40" role="region" aria-labelledby="pie-chart-title">
  ...
  return (
    <figure className="w-full h-64 sm:h-80" role="region" aria-labelledby="line-chart-title">
      <figcaption id="line-chart-title" className="sr-only">
        進度趨勢線圖表包含 {chartData.combinedData.filter(d => d.progress !== null).length} 個實際資料點及{chartData.hasPrediction ? '10天預測' : '無預測'}
      </figcaption>
      <div className="text-xs text-gray-600 mb-2 text-center sm:text-left">
        資料點數量: {chartData.combinedData.filter(d => d.progress !== null).length} {chartData.hasPrediction ? '| 預測: 10天' : ''}
      </div>
        ...
        >
          ...
          >
            ...
            <XAxis
              ...
              textAnchor="end"
              height={60}
              interval={Math.max(0, Math.floor(chartData.combinedData.length / 10) - 1)}
            />
            <YAxis
              tick={{ fontSize: 10 }}
              domain={[0, 100]}
            />
            <Tooltip
              formatter={(value, name) => [`${value?.toFixed(2)}%`, name === 'progress' ? '實際進度' : '預測進度']}
