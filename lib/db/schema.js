...
// 15. character_abilities — one row per inner-ability line per preset.
export const characterAbilities = mysqlTable(
  'character_abilities',
  {
    id: bigint('id', { mode: 'number' }).autoincrement().primaryKey(),
    ocid: varchar('ocid', { length: 64 })
      .notNull()
      .references(() => characters.ocid, { onDelete: 'cascade' }),
    presetNo: int('preset_no').notNull(),
    abilityNo: varchar('ability_no', { length: 10 }),
    abilityGrade: varchar('ability_grade', { length: 20 }),
    abilityValue: varchar('ability_value', { length: 200 }),
    presetGrade: varchar('preset_grade', { length: 20 }),
    updatedAt: timestamp('updated_at').defaultNow().onUpdateNow(),
  },
  table => [index('idx_ocid').on(table.ocid)]
);

// 15b. character_ability_meta — active preset number, overall grade,
// and remaining fame (one row per character).
export const characterAbilityMeta = mysqlTable(
  'character_ability_meta',
  {
    id: bigint('id', { mode: 'number' }).autoincrement().primaryKey(),
    ocid: varchar('ocid', { length: 64 })
      .notNull()
      .references(() => characters.ocid, { onDelete: 'cascade' }),
    activePresetNo: int('active_preset_no'),
    abilityGrade: varchar('ability_grade', { length: 20 }),
    remainFame: bigint('remain_fame', { mode: 'number' }),
    updatedAt: timestamp('updated_at').defaultNow().onUpdateNow(),
  },
  table => [uniqueIndex('uk_ocid').on(table.ocid)]
);
