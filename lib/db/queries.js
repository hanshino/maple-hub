import { asc, eq, desc, like, and, sql } from 'drizzle-orm';
import { getDb } from './index.js';
import {
  characters,
  characterStats,
  characterEquipment,
  characterCurrentEquipment,
...
export async function getFullCharacterData(ocid) {
  ...
  const [
    ...
    db
      .select()
      .from(characterCurrentEquipment)
      .where(eq(characterCurrentEquipment.ocid, ocid))
      .orderBy(asc(characterCurrentEquipment.sourceOrdinal)),
    db
      .select()
      .from(characterEquipmentPresets)
      .where(eq(characterEquipmentPresets.ocid, ocid)),
    db
      .select()
    ...
    db
      .select()
      .from(characterFamiliars)
      .where(eq(characterFamiliars.ocid, ocid)),
    db
      .select()
      .from(characterAbilities)
      .where(eq(characterAbilities.ocid, ocid)),
    db
      .select()
      .from(characterAbilityMeta)
      .where(eq(characterAbilityMeta.ocid, ocid)),
  ]);

  // Map equipment rows back to Nexon API format, grouped by preset.
  const activeEquipmentPresetNo = equipPresetRow[0]?.activePresetNo || 1;
  ...
  const equipmentData = {
    preset_no: activeEquipmentPresetNo,
    item_equipment: [],
  ...
  for (const row of equipRows) {
    ...
    if (equipmentData[key]) equipmentData[key].push(mapEquipmentRow(row));
  }
  equipmentData.item_equipment = equipPresetRow[0]?.hasCurrentEquipmentSnapshot
    ? currentEquipRows.map(mapEquipmentRow)
    : currentEquipRows.length > 0
      ? currentEquipRows.map(mapEquipmentRow)
      : equipmentData[`item_equipment_preset_${activeEquipmentPresetNo}`] ||
        equipmentData.item_equipment_preset_1;

  const equipmentPresetsData = {
    active: activeEquipmentPresetNo,
    presets: {
  ...
  const linkSkillData = {
    ...
    character_owned_link_skill: linkPresetRow[0]?.ownedLinkSkill || null,
  };
  ...
  for (const row of petEquipRows) {
    ...
    if (slot) {
      ...
      petEquipData[`pet_${num}_nickname`] = row.petNickname;
      petEquipData[`pet_${num}_icon`] = row.petIcon;
      petEquipData[`pet_${num}_description`] = row.petDescription;
      petEquipData[`pet_${num}_pet_type`] = row.petType;
      petEquipData[`pet_${num}_equipment`] = row.petTotalOption;
      petEquipData[`pet_${num}_auto_skill`] = row.petAutoSkill;
      petEquipData[`pet_${num}_skill`] = row.petSkill || [];
      petEquipData[`pet_${num}_date_expire`] = row.petDateExpire;
      petEquipData[`pet_${num}_appearance`] = row.petAppearance;
      petEquipData[`pet_${num}_appearance_icon`] = row.petAppearanceIcon;
    }
  }
  ...
  const unionChampionData = {
    union_champion: unionChampionRows.map(row => ({
      ...
      champion_ocid: row.championOcid || null,
      champion_image: row.championImage || null,
  ...
  // Map inner ability back to Nexon format: active lines at the top
  // level plus the three presets. Absent rows mean the character was
  // synced before ability support existed → null, never invented.
  const abilityMeta = abilityMetaRows[0] || null;
  const abilityLine = row => ({
    ability_no: row.abilityNo,
    ability_grade: row.abilityGrade,
    ability_value: row.abilityValue,
  });
  const abilityPreset = presetNo => {
    const rows = abilityRows.filter(r => r.presetNo === presetNo);
    return {
      ability_preset_grade: rows[0]?.presetGrade ?? null,
      ability_info: rows.map(abilityLine),
    };
  };
  const abilityData = abilityMeta
    ? {
        ability_grade: abilityMeta.abilityGrade,
        preset_no: abilityMeta.activePresetNo,
        remain_fame: abilityMeta.remainFame,
        ability_info: abilityPreset(abilityMeta.activePresetNo).ability_info,
        ability_preset_1: abilityPreset(1),
        ability_preset_2: abilityPreset(2),
        ability_preset_3: abilityPreset(3),
      }
    : null;

  // Map familiars back to Nexon format. Linked familiars carry
  // familiar_state='linked'; absence of rows means the character was
  // synced before familiar support existed, so the payload stays null
  // (unknown, never zero).
  const familiarData =
    familiarRows.length > 0
      ? {
          familiar_info: familiarRows.map(row => ({
            familiar_name: row.familiarName,
            familiar_level: row.familiarLevel,
            option_level: row.optionLevel,
            familiar_grade: row.familiarGrade,
            familiar_state: row.familiarState,
            slot_id: row.slotId,
            familiar_special_flag: row.specialFlag ? 'true' : 'false',
            summoned_flag: row.summonedFlag ? 'true' : 'false',
            option: row.familiarOption || [],
          })),
        }
      : null;
  ...
  return {
    ...
    equipment_presets: equipmentPresetsData,
...
function mapEquipmentItem(ocid, item) {
  return {
    ocid,
...
export async function replaceEquipmentSnapshot(
  ocid,
  activePresetNo,
  currentItems,
  presets
  ...
  await db.transaction(async tx => {
    await tx
      .insert(characterEquipmentPresets)
      .values({ ocid, activePresetNo, hasCurrentEquipmentSnapshot: 1 })
      .onDuplicateKeyUpdate({
        set: {
          activePresetNo,
    ...
    await tx
      .delete(characterCurrentEquipment)
      .where(eq(characterCurrentEquipment.ocid, ocid));
    await tx
    ...
    if (currentItems.length > 0) {
      await tx.insert(characterCurrentEquipment).values(
        currentItems.map((item, sourceOrdinal) => ({
          ...mapEquipmentItem(ocid, item),
          sourceOrdinal,
        }))
      );
    }

    const presetRows = [1, 2, 3].flatMap(presetNo =>
      (presets[presetNo] || []).map(item => ({
        ...mapEquipmentItem(ocid, item),
        presetNo,
      }))
    );
    if (presetRows.length > 0) {
      await tx.insert(characterEquipment).values(presetRows);
    }
  });
}
...
export async function upsertEquipment(ocid, presetNo, items) {
  ...
  await db
    ...
    .values(items.map(item => ({ ...mapEquipmentItem(ocid, item), presetNo })));
}

export async function upsertEquipmentPreset(ocid, activePresetNo) {
  const db = getDb();
  await db
    .insert(characterEquipmentPresets)
    .values({ ocid, activePresetNo })
    .onDuplicateKeyUpdate({
      set: { activePresetNo, updatedAt: sql`NOW()` },
    });
}
...
export async function upsertLinkSkillPreset(ocid, usePresetNo, ownedLinkSkill) {
  const db = getDb();
  await db
    .insert(characterLinkSkillPresets)
    .values({ ocid, usePresetNo, ownedLinkSkill: ownedLinkSkill || null })
    .onDuplicateKeyUpdate({
      set: {
        usePresetNo,
        ownedLinkSkill: ownedLinkSkill || null,
        updatedAt: sql`NOW()`,
      },
    });
}
...
export async function upsertUnionChampion(ocid, champions, totalBadgeInfo) {
  ...
  if (totalBadgeInfo) {
    for (const badge of totalBadgeInfo) {
      badgeRows.push({ ocid, championSlot: null, stat: badge.stat });
    }
  }

  await Promise.all([
    db.insert(characterUnionChampion).values(
      champions.map(c => ({
        ocid,
        championSlot: c.champion_slot,
        ...
        championOcid: c.champion_ocid || null,
        championImage: c.champion_image || null,
      }))
    ),
    badgeRows.length > 0
      ? db.insert(characterUnionChampionBadges).values(badgeRows)
      : Promise.resolve(),
  ]);
}
...
// Nexon returns the literal string "expired" for a lapsed pet magic-time
// instead of a date — guard against that (and any other unparsable value).
...
export async function upsertPetEquipment(ocid, pets) {
  ...
  await db.insert(characterPetEquipment).values(
    pets.map(p => ({
      ...
      petNickname: p.pet_nickname || null,
      petIcon: p.pet_icon || null,
      petDescription: p.pet_description || null,
      petType: p.pet_type || null,
      petEquipmentSlot: p.pet_equipment_slot || null,
      petTotalOption: p.pet_total_option || null,
      petAutoSkill: p.pet_auto_skill || null,
      petSkill: p.pet_skill || null,
      petDateExpire: parseValidDate(p.pet_date_expire),
      petAppearance: p.pet_appearance || null,
      petAppearanceIcon: p.pet_appearance_icon || null,
    }))
  );
}

export async function upsertFamiliars(ocid, familiarInfo) {
  const db = getDb();
  await db.delete(characterFamiliars).where(eq(characterFamiliars.ocid, ocid));
  if (!familiarInfo || familiarInfo.length === 0) return;

  await db.insert(characterFamiliars).values(
    familiarInfo.map(f => ({
      ocid,
      familiarName: f.familiar_name || null,
      familiarLevel: f.familiar_level || 0,
      optionLevel: f.option_level || 0,
      familiarGrade: f.familiar_grade || null,
      familiarState: f.familiar_state || null,
      slotId: f.slot_id || null,
      specialFlag: f.familiar_special_flag === 'true' ? 1 : 0,
      summonedFlag: f.summoned_flag === 'true' ? 1 : 0,
      familiarOption: f.option || [],
    }))
  );
}

export async function upsertAbilities(ocid, abilityData) {
  const db = getDb();
  await Promise.all([
    db.delete(characterAbilities).where(eq(characterAbilities.ocid, ocid)),
    db.delete(characterAbilityMeta).where(eq(characterAbilityMeta.ocid, ocid)),
  ]);

  await db.insert(characterAbilityMeta).values({
    ocid,
    activePresetNo: abilityData.preset_no ?? null,
    abilityGrade: abilityData.ability_grade || null,
    remainFame: abilityData.remain_fame ?? null,
  });

  const lineRows = [];
  for (const presetNo of [1, 2, 3]) {
    const preset = abilityData[`ability_preset_${presetNo}`];
    for (const line of preset?.ability_info || []) {
      lineRows.push({
        ocid,
        presetNo,
        abilityNo: line.ability_no || null,
        abilityGrade: line.ability_grade || null,
        abilityValue: line.ability_value || null,
        presetGrade: preset.ability_preset_grade || null,
      });
    }
  }
  // Older/degraded responses may carry only the active line list —
  // keep those under the active preset number so nothing is dropped.
  if (lineRows.length === 0 && Array.isArray(abilityData.ability_info)) {
    const presetNo = abilityData.preset_no || 1;
    for (const line of abilityData.ability_info) {
      lineRows.push({
        ocid,
        presetNo,
        abilityNo: line.ability_no || null,
        abilityGrade: line.ability_grade || null,
        abilityValue: line.ability_value || null,
        presetGrade: abilityData.ability_grade || null,
      });
    }
  }
  if (lineRows.length > 0) {
    await db.insert(characterAbilities).values(lineRows);
  }
}
