...
export async function getFullCharacterData(ocid) {
  ...
  const abilityData = abilityMeta
    ? {
        ...
        ability_info: abilityRows
          .filter(r => r.presetNo === abilityMeta.activePresetNo)
          .map(abilityLine),
...
export async function upsertAbilities(ocid, abilityData) {
  const db = getDb();
  await db.delete(characterAbilities).where(eq(characterAbilities.ocid, ocid));
  await db
    .delete(characterAbilityMeta)
    .where(eq(characterAbilityMeta.ocid, ocid));
