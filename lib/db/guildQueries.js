...
export async function getGuildByNameAndWorld(guildName, worldName) {
  const db = getDb();
  const [guild] = await db
    .select()
    .from(guilds)
    .where(
      and(eq(guilds.guildName, guildName), eq(guilds.worldName, worldName))
    )
    .limit(1);
  return guild || null;
}
...
export async function upsertCharacterBasicOnly(data) {
  ...
  const updateSet = {
    ...
    characterGender: data.characterGender,
    worldName: data.worldName,
    // Do NOT update updatedAt here — it's used by character API to check
    // if a full sync is needed. Guild basic-only writes should not reset it.
  };
...
export async function getExpSnapshots(oguildId, dates) {
  ...
  for (const row of rows) {
    ...
    // Drizzle date() returns Date objects — normalize to YYYY-MM-DD string
