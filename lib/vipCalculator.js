...
/**
 * @param {object} inputs
 * @param {number} [inputs.giftRate]  送禮轉換率（點/樂豆）覆寫；有傳＝平率，沒傳＝依 LEVELS 分段（相容舊介面）
...
export function computeVip(inputs = {}) {
  ...
  // 比率解析：giftRate 有傳＝平率覆寫（相容舊介面），此時不輸出分段；沒傳＝依 LEVELS 分段。
  // 自用一律走分段（seg.leadouSelf 與 parts 的 ldSelf 恆一致，無平率覆寫）。
  ...
  const leadouSelf = seg.leadouSelf;
