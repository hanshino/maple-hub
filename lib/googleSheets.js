...
class GoogleSheetsClient {
  ...
  async getLeaderboardData(offset = 0, limit = 20, filters = {}) {
    ...
    try {
      ...
      const { characterInfoMap, search, worldName, characterClass } = filters;
      ...
      if (characterInfoMap && hasFilters) {
        filtered = entries.filter(entry => {
          ...
          if (
            search &&
            !info.character_name.toLowerCase().includes(search.toLowerCase())
          ) {
            return false;
          }
  ...
  async removeOcids(ocids) {
    ...
    try {
      console.log(`🗑️ Removing ${ocids.length} OCIDs from all sheets...`);
      ...
      for (const config of sheetConfigs) {
        ...
        try {
          ...
          console.warn(`⚠️ Could not process ${config.name} sheet for removal`);
        }
      }
