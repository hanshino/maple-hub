...
import { STAT_ALIASES } from './statNames.js';
...
// Flat attack/magic sums exposed as equipment-category rows, one per
// normalized-equipment field of the same name.
const EQUIPMENT_SUM_METRICS = [
  'baseAttackSum',
  'baseMagicSum',
  'starforceAttackSum',
  'starforceMagicSum',
...
// Sums option_value across `{ option_type, option_value }` lists (pet
// equipment item_option, cash item cash_item_option) for one stat name.
function sumTypedOptions(lists, statName) {
  return lists.reduce(
    (sum, list) =>
      sum +
      (Array.isArray(list) ? list : []).reduce(
        (optSum, o) =>
          o?.option_type === statName
...
export function normalizeCharacterForComparison(data) {
  ...
  const str = getStat(...STAT_ALIASES.STR);
  const dex = getStat(...STAT_ALIASES.DEX);
  const intStat = getStat(...STAT_ALIASES.INT);
  const luk = getStat(...STAT_ALIASES.LUK);
  ...
  const finalStats = {
    ...
    values: {
      ...
      damage: getStat(...STAT_ALIASES.damage),
      bossDamage: getStat(...STAT_ALIASES.bossDamage),
      ied: getStat(...STAT_ALIASES.ied),
      critRate: getStat(...STAT_ALIASES.critRate),
      critDamage: getStat(...STAT_ALIASES.critDamage),
      starForceFinal: getStat(...STAT_ALIASES.starForceFinal),
      authenticForce: getStat(...STAT_ALIASES.authenticForce),
    },
  };
  ...
  // Attack/magic flat sums by source across the active items. The star
  // count sum alone badly understates gear gaps (attack per star scales
  // with item level), so each enhancement source is summed separately.
  const equipSum = (optionKey, field) =>
    hasEquipment
      ? sumOptionField(activeEquipmentItems, optionKey, field)
      : null;
  // Pet equipment / cash items also contribute flat attack the equipment
  // list never shows. They have been synced since the project began, so
  // when equipment data exists, an absent pet/cash payload is a real
  // zero (no pets / stat-less cash items), not missing coverage.
  const petItemOptions = [1, 2, 3].map(
    n => data?.petEquipment?.[`pet_${n}_equipment`]?.item_option
  );
  const cashItemOptions = (
    data?.cashEquipment?.cash_item_equipment_base || []
  ).map(c => c?.cash_item_option);
  const equipment = {
    ...
    baseAttackSum: equipSum('item_base_option', 'attack_power'),
    baseMagicSum: equipSum('item_base_option', 'magic_power'),
    starforceAttackSum: equipSum('item_starforce_option', 'attack_power'),
    starforceMagicSum: equipSum('item_starforce_option', 'magic_power'),
    scrollAttackSum: equipSum('item_etc_option', 'attack_power'),
    scrollMagicSum: equipSum('item_etc_option', 'magic_power'),
    flameAttackSum: equipSum('item_add_option', 'attack_power'),
    flameMagicSum: equipSum('item_add_option', 'magic_power'),
    totalAttackSum: equipSum('item_total_option', 'attack_power'),
    totalMagicSum: equipSum('item_total_option', 'magic_power'),
    petAttackSum: hasEquipment
      ? sumTypedOptions(petItemOptions, '攻擊力')
      : null,
    petMagicSum: hasEquipment
      ? sumTypedOptions(petItemOptions, '魔法攻擊力')
      : null,
    cashAttackSum: hasEquipment
      ? sumTypedOptions(cashItemOptions, '攻擊力')
      : null,
    cashMagicSum: hasEquipment
      ? sumTypedOptions(cashItemOptions, '魔法攻擊力')
      : null,
  };
  ...
  // Inner ability (內在能力): the active preset's lines are API facts;
  // the combat-relevant numbers are parsed out of the line text. A line
  // that matches no pattern (utility lines like cooldown skip or drop
  // rate) simply contributes nothing — the tab still shows the raw
  // text, so nothing is hidden. The 狀態異常/一般怪物 damage lines must
  // not count as boss damage, hence the explicit Boss怪物 anchor.
  const abilityInfo = data?.ability?.ability_info;
  const hasAbility = Array.isArray(abilityInfo) && abilityInfo.length > 0;
  const abilityLineSum = pattern =>
    abilityInfo.reduce((sum, line) => {
      const m = String(line?.ability_value || '').match(pattern);
      return sum + (m ? (numOrNull(m[1]) ?? 0) : 0);
    }, 0);
  const ability = {
    coverage: hasAbility,
    grade: hasAbility ? (data.ability.ability_grade ?? null) : null,
    activePresetNo: hasAbility ? presetNoOrNull(data.ability.preset_no) : null,
    bossDamage: hasAbility
      ? abilityLineSum(/Boss怪物時[，,]?\s*傷害增加\s*([\d.]+)\s*%/)
      : null,
    attackPower: hasAbility
      ? abilityLineSum(/^攻擊力增加\s*([\d.]+)\s*$/)
      : null,
    magicPower: hasAbility
      ? abilityLineSum(/^魔法攻擊力增加\s*([\d.]+)\s*$/)
      : null,
    critRate: hasAbility ? abilityLineSum(/爆擊機率增加\s*([\d.]+)\s*%/) : null,
  };
...
export function compareCharacters(left, right) {
  ...
  // `rows` carries only 'stats'-category entries — the only category
  // components/compare/KeyStatTable.js reads. Summary data (level,
  // world, union level, symbol progress, snapshot quality) lives on
  // `left`/`right`/`snapshot`/`categories` instead of being duplicated
  // here.
  const rows = [];
  ...
  const categories = {
    equipment: {
      ...
      // Flat attack/magic sums by source (equipment options plus pet
      // equipment and cash items) — all derived, same row shape.
      ...Object.fromEntries(
        EQUIPMENT_SUM_METRICS.map(metric => [
          metric,
          makeRow({
            category: 'Equipment',
            metric,
            left: left?.equipment?.[metric] ?? null,
            right: right?.equipment?.[metric] ?? null,
            unit: null,
            evidence: 'derived',
          }),
        ])
    ...
    ability: {
      activePresetNo: {
        left: left?.ability?.activePresetNo ?? null,
        right: right?.ability?.activePresetNo ?? null,
      },
      bossDamage: makeRow({
        category: 'Ability',
        metric: 'abilityBossDamage',
        left: left?.ability?.bossDamage ?? null,
        right: right?.ability?.bossDamage ?? null,
        unit: 'pp',
        evidence: 'derived',
      }),
      attackPower: makeRow({
        category: 'Ability',
        metric: 'abilityAttackPower',
        left: left?.ability?.attackPower ?? null,
        right: right?.ability?.attackPower ?? null,
        unit: null,
        evidence: 'derived',
      }),
      magicPower: makeRow({
        category: 'Ability',
        metric: 'abilityMagicPower',
        left: left?.ability?.magicPower ?? null,
        right: right?.ability?.magicPower ?? null,
        unit: null,
        evidence: 'derived',
      }),
      critRate: makeRow({
        category: 'Ability',
        metric: 'abilityCritRate',
        left: left?.ability?.critRate ?? null,
        right: right?.ability?.critRate ?? null,
        unit: 'pp',
        evidence: 'derived',
      }),
    },
...
export const HEADLINE_METRIC_BY_CATEGORY = {
  ...
  Ability: categories => categories.ability.bossDamage,
};
...
export function rankUpgradeDirections(comparison) {
  ...
  for (const category of CATEGORY_ORDER) {
    const row = HEADLINE_METRIC_BY_CATEGORY[category](comparison.categories);
