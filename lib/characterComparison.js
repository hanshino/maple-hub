...
export function normalizeCharacterForComparison(data) {
  ...
  const hexa = {
    ...
    totalCoreLevel: hasHexaCores
      ...
      : null,
    // Authentic Force is an API-fact final-stat figure, not derived from
    // hexaCores/hexaStats — it is the headline HEXA metric this module
    // uses for upgrade-direction eligibility.
  ...
  const categories = {
    ...
    hexa: {
      authenticForce: makeRow({
        ...
        left: left?.hexa?.authenticForce ?? null,
        right: right?.hexa?.authenticForce ?? null,
        unit: null,
        evidence: 'api-fact',
      }),
    },
...
const HEADLINE_ROW_BY_CATEGORY = {
  ...
  HEXA: categories => categories.hexa.authenticForce,
