...
export function normalizeCharacterForComparison(data) {
  ...
  // Familiars: registered breadth plus what actually applies in combat —
  // the linked ones and their option levels. Characters synced before
  // familiar support have no payload at all → coverage false, never zero.
  ...
  const familiar = {
    ...
    // A character with familiar data but nothing linked has a real zero
    // here, not an unknown.
    linkedOptionLevelSum: hasFamiliars
      ? (sumField(linkedFamiliars, 'option_level') ?? 0)
      : null,
  };
...
export function compareCharacters(left, right) {
  ...
  const categories = {
    ...
    familiar: {
      ...
      linkedOptionLevelSum: makeRow({
        category: 'Familiar',
        metric: 'linkedFamiliarOptionLevelSum',
        left: left?.familiar?.linkedOptionLevelSum ?? null,
        right: right?.familiar?.linkedOptionLevelSum ?? null,
        unit: null,
        evidence: 'derived',
      }),
    },
...
const HEADLINE_ROW_BY_CATEGORY = {
  ...
  Familiar: categories => categories.familiar.registeredCount,
};
