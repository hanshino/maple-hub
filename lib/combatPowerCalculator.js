 ...
 */
/**
 * Counts how many leveling keyword matches exist across all items' potential fields.
 * @param {object[]} presetItems
 * @returns {number}
 */
export function countLevelingKeywords(presetItems) {
  let count = 0;
...
export function identifyLevelingPreset(presetItems) {
  return countLevelingKeywords(presetItems) >= LEVELING_THRESHOLD;
}
...
export function analyzePresetCombinations(
  ...
  const live = {
    ...
    hyperStat: hyperStatData?.use_preset_no
      ...
      : null,
    linkSkill: linkSkillData?.use_preset_no
      ? parseInt(linkSkillData.use_preset_no)
      : null,
  };

  // Classify each equipment preset.
  // Only the preset with the MOST leveling keywords (and >= threshold) is 'exp'.
  // This prevents bossing presets that happen to have a few drop-rate potentials
  // from being misclassified as leveling.
  const equipClassify = {};
  const equipLevelingCounts = {};
  for (let i = 1; i <= 3; i++) {
    const items = equipmentData?.[`item_equipment_preset_${i}`] || [];
    equipLevelingCounts[i] = countLevelingKeywords(items);
  }
  const maxLevelingNo = [1, 2, 3].reduce((best, i) =>
    equipLevelingCounts[i] > equipLevelingCounts[best] ? i : best
  );
  for (let i = 1; i <= 3; i++) {
    const items = equipmentData?.[`item_equipment_preset_${i}`] || [];
    if (items.length === 0) {
      ...
      i === maxLevelingNo &&
      equipLevelingCounts[i] >= LEVELING_THRESHOLD
    ) {
      equipClassify[i] = 'exp';
    } else {
      equipClassify[i] = 'boss';
    }
  }
