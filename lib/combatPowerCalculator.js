...
export function parsePotentialOption(str) {
  ...
  const pctStatMatch = str.match(/^(STR|DEX|INT|LUK|攻擊力|魔法攻擊力)\s*:\s*\+(\d+)%/);
  if (pctStatMatch) {
    return { stat: pctStatMatch[1], value: parseInt(pctStatMatch[2]), isPercent: true };
  }

  // Flat stat (STR/DEX/INT/LUK/攻擊力/魔法攻擊力)
  const flatStatMatch = str.match(/^(STR|DEX|INT|LUK|攻擊力|魔法攻擊力)\s*:\s*\+(\d+)$/);
  if (flatStatMatch) {
    return { stat: flatStatMatch[1], value: parseInt(flatStatMatch[2]), isPercent: false };
  }
...
export function identifyIndependentItems(equipmentData) {
  ...
  const hasAnyPreset = [item_equipment_preset_1, item_equipment_preset_2, item_equipment_preset_3].some(p => p && p.length > 0);
  if (!hasAnyPreset) return [];
  for (const preset of [item_equipment_preset_1, item_equipment_preset_2, item_equipment_preset_3]) {
    if (!preset) continue;
 ...
 *   CP ≈ (main×4 + sub) × 1.3 × ATK/100 × (1 + DMG% + Boss%) × (1 + CritDMG%)
 *
 ...
 * @returns {number}
 */
function calculatePresetCombatPower(targetPresetItems, currentPresetItems, independentItems, statsData, symbolData) {
  ...
  const apiATK = getStat('공격력') || getStat('攻擊力') || getStat('物理攻擊力');
  const apiMATK = getStat('魔法攻擊力');

  // Calculate C_final from symbols (100 + level*100 per symbol)
  const symbols = symbolData?.symbol || [];
  const cFinalLUK = symbols.reduce((acc, sym) => acc + 100 + (parseInt(sym.symbol_level) || 0) * 100, 0);
  ...
  const cBaseLUK = calcCBase(apiLUK, cFinalLUK, currentStats.fixed.LUK, currentStats.percent.LUK);
  const cBaseSTR = calcCBase(apiSTR, cFinalSTR, currentStats.fixed.STR, currentStats.percent.STR);
  const cBaseDEX = calcCBase(apiDEX, cFinalDEX, currentStats.fixed.DEX, currentStats.percent.DEX);
  const cBaseINT = calcCBase(apiINT, cFinalINT, currentStats.fixed.INT, currentStats.percent.INT);
  ...
  const cBaseATK = calcAtkCBase(apiATK, currentStats.fixed.attack_power, currentStats.percent.attack_power);
  const cBaseMATK = calcAtkCBase(apiMATK, currentStats.fixed.magic_power, currentStats.percent.magic_power);

  // Get damage/boss/crit from current items (used as baseline if new preset doesn't change them)
  const currentDmgPct = currentStats.percent.damage;
  const currentBossPct = currentStats.percent.boss_damage;
  const currentCritPct = currentStats.percent.critical_damage;

  // Extract stats for the target preset
  const targetStats = extractEquipmentStats([...targetPresetItems, ...independentItems]);

  const newLUK = Math.floor((cBaseLUK + targetStats.fixed.LUK) * (1 + targetStats.percent.LUK / 100)) + cFinalLUK;
  const newSTR = Math.floor((cBaseSTR + targetStats.fixed.STR) * (1 + targetStats.percent.STR / 100)) + cFinalSTR;
  const newDEX = Math.floor((cBaseDEX + targetStats.fixed.DEX) * (1 + targetStats.percent.DEX / 100)) + cFinalDEX;
  const newINT = Math.floor((cBaseINT + targetStats.fixed.INT) * (1 + targetStats.percent.INT / 100)) + cFinalINT;

  const newATK = Math.floor((cBaseATK + targetStats.fixed.attack_power) * (1 + targetStats.percent.attack_power / 100));
  const newMATK = Math.floor((cBaseMATK + targetStats.fixed.magic_power) * (1 + targetStats.percent.magic_power / 100));

  const newDmgPct = targetStats.percent.damage;
  const newBossPct = targetStats.percent.boss_damage;
  const newCritPct = targetStats.percent.critical_damage;

  // Determine main/sub stats (use highest as main for estimation)
  ...
  // Use preset-specific damage values if available, else fallback to current
  const dmg = newDmgPct > 0 ? newDmgPct : currentDmgPct;
  const boss = newBossPct > 0 ? newBossPct : currentBossPct;
  const crit = newCritPct > 0 ? newCritPct : currentCritPct;

  // CP formula (section 11.2)
  const cp = (main * 4 + sub) * 1.3 * (atk / 100) * (1 + (dmg + boss) / 100) * (1 + crit / 100);
