...
function calculateMaxRequirements() {
  // Equipment cores max costs
  const skillCoreMax = { soul_elder: 145, soul_elder_fragment: 4400 }; // 1 skill core 0~30
  const sharedCoreMax = { soul_elder: 208, soul_elder_fragment: 6268 }; // 1 shared core 0~30
  const masteryCoreMax = { soul_elder: 83, soul_elder_fragment: 2252 }; // 1 mastery core 0~20
  const enhancementCoreMax = { soul_elder: 123, soul_elder_fragment: 3383 }; // 1 enhancement core 0~20

  // Stat cores max activation costs
  const statCoreMax = {
    soul_elder: 5 + 10 + 15,
    soul_elder_fragment: 10 + 200 + 350,
  }; // 3 stat cores activation

  const totalSoulElder =
    skillCoreMax.soul_elder +
    sharedCoreMax.soul_elder +
    masteryCoreMax.soul_elder * 4 + // 4 mastery cores
    enhancementCoreMax.soul_elder * 4 + // 4 enhancement cores
    statCoreMax.soul_elder;

  const totalSoulElderFragment =
    skillCoreMax.soul_elder_fragment +
    sharedCoreMax.soul_elder_fragment +
    masteryCoreMax.soul_elder_fragment * 4 + // 4 mastery cores
    enhancementCoreMax.soul_elder_fragment * 4 + // 4 enhancement cores
    statCoreMax.soul_elder_fragment;
...
export function calculateHexaMatrixProgress(hexaData) {
  ...
  // Calculate stat core costs - based on number of activated slots
  const statCoreCosts = calculateStatCoreCosts(statCoresCount, statCores);
  totalSpentSoulElder += statCoreCosts.soul_elder;
  totalSpentSoulElderFragment += statCoreCosts.soul_elder_fragment;

  // Calculate max requirements based on total available cores
  // 1 skill core, 1 shared core, 4 mastery cores, 4 enhancement cores, 3 stat cores
  const maxRequirements = calculateMaxRequirements();
  totalRequiredSoulElder = maxRequirements.soul_elder;
  totalRequiredSoulElderFragment = maxRequirements.soul_elder_fragment;

  // Calculate progress primarily based on stat core fragments only (as per user specification)
  // Progress = (stat core fragments spent) / (total available fragments) * 100
  const totalProgress =
    totalRequiredSoulElderFragment > 0
      ? (statCoreCosts.soul_elder_fragment / totalRequiredSoulElderFragment) *
        100
      : 0;
 ...
 */
function calculateTotalCostUpToLevel(level, coreType) {
...
/**
 * Get total cost for max level (30)
 * @param {string} coreType - Core type key
 * @returns {Object} Total cost for max level
 */
function getMaxLevelTotalCost(coreType) {
  return calculateTotalCostUpToLevel(30, coreType);
}

/**
 * Calculate stat core costs - only activation costs
 * @param {number} activatedSlotsCount - Number of activated stat core slots
 * @param {Array} allStatCores - All stat cores from activated slots
 * @returns {Object} Total costs for stat cores (only activation)
 */
function calculateStatCoreCosts(activatedSlotsCount, allStatCores) {
  let totalSoulElder = 0;
  let totalSoulElderFragment = 0;

  // Only calculate activation costs for each activated slot
  for (let slotIndex = 0; slotIndex < activatedSlotsCount; slotIndex++) {
    // Activation cost based on core tier
    const activationCost =
      STAT_CORE_ACTIVATION_COSTS[slotIndex] ||
