...
export function calculateEstimatedTimeToLevel(
  ...
  if (historicalData && historicalData.length >= 2) {
    // Normalize entries to a numeric percentage value (0-100) and calculate cumulative progress
    const points = historicalData
      .map(item => {
        if (!item || !item.date) return null;
        let percentage = 0;
        if (typeof item.progress === 'number') {
          // legacy: progress is 0-1 -> convert to 0-100
        ...
        // Calculate cumulative progress considering level changes
        const level = item.level || 1;
        const cumulativeProgress = percentage; // We'll adjust this after finding baseline
    ...
    if (points.length >= 2) {
      // Find baseline level (lowest level in the dataset)
      const baselineLevel = Math.min(...points.map(p => p.level));

      // Recalculate cumulative progress relative to baseline level
      points.forEach(point => {
        const levelAdjustment = (point.level - baselineLevel) * 100;
        point.cumulativeProgress = point.percentage + levelAdjustment;
      });
      ...
      const daysDiff =
        (new Date(lastPoint.date) - new Date(firstPoint.date)) /
        (1000 * 60 * 60 * 24);
      const progressDiff =
        lastPoint.cumulativeProgress - firstPoint.cumulativeProgress;

      if (daysDiff > 0 && progressDiff > 0) {
        const dailyGrowthRate = progressDiff / daysDiff;

        // 計算達到 100%（從當前剩餘百分比）的天數
        const daysNeeded = remainingPercentage / dailyGrowthRate;
        const hours = daysNeeded * 24; // 轉換為小時
        return hours;
      }
    }
  }

  // 回退到舊邏輯：假設每小時固定獲得 expRatePerHour% 的經驗值
  const fallbackHours = remainingPercentage / expRatePerHour;
 ...
 */
function calculateUpgradeCosts(_core) {
