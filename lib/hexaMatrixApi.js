...
// Cache key for hexa stat core data
const HEXA_STAT_CORE_CACHE_KEY = 'hexa_stat_core_cache';

/**
 * Fetch Hexa Stat Core data for a character
 * @param {string} ocid - Character OCID
 * @returns {Promise<Object>} Hexa Stat Core data
 */
export async function fetchHexaStatCores(ocid) {
  ...
  // Check cache first
  const cached = getCachedHexaStatCoreData(ocid);
  if (cached) {
    return cached;
  }
  ...
  try {
    ...
    const response = await axios.get(`/api/hexa-matrix-stat`, {
      params: { ocid },
      headers: {
        accept: 'application/json',
      },
    ...
    // Cache the result
    cacheHexaStatCoreData(ocid, response.data);
...
/**
 * Get cached Hexa Stat Core data for a character
 * @param {string} ocid - Character OCID
 * @returns {Object|null} Cached data or null if not found/expired
 */
function getCachedHexaStatCoreData(ocid) {
  try {
    const cache = JSON.parse(
      localStorage.getItem(HEXA_STAT_CORE_CACHE_KEY) || '{}'
    );
    const characterCache = cache[ocid];
    ...
    if (now - characterCache.timestamp > CACHE_DURATION) {
      // Cache expired, remove it
      delete cache[ocid];
      localStorage.setItem(HEXA_STAT_CORE_CACHE_KEY, JSON.stringify(cache));
      return null;
    }
...
/**
 * Cache Hexa Stat Core data for a character
 * @param {string} ocid - Character OCID
 * @param {Object} data - Data to cache
 */
function cacheHexaStatCoreData(ocid, data) {
  try {
    const cache = JSON.parse(
      localStorage.getItem(HEXA_STAT_CORE_CACHE_KEY) || '{}'
    );
    ...
    // If caching fails, silently continue
    console.warn('Failed to cache Hexa Stat Core data:', error);
  }
}

/**
 * Clear all cached Hexa Stat Core data
 */
export function clearHexaStatCoreCache() {
  try {
    localStorage.removeItem(HEXA_STAT_CORE_CACHE_KEY);
  } catch (error) {
    console.warn('Failed to clear Hexa Stat Core cache:', error);
  }
}
