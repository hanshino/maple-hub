...
export async function syncCharacter(ocid) {
  try {
    ...
    // 3. Replace equipment snapshot (current list + 3 presets) atomically.
    const equip = val(equipData);
    if (equip) {
      const activePresetNo = parseInt(equip.preset_no) || 1;
      await replaceEquipmentSnapshot(
        ocid,
        activePresetNo,
        equip.item_equipment || [],
        {
          1: equip.item_equipment_preset_1 || [],
          2: equip.item_equipment_preset_2 || [],
          3: equip.item_equipment_preset_3 || [],
        }
      );
    }
    ...
    if (link) {
      ...
      await upsertLinkSkillPreset(
        ocid,
        presetNo,
        link.character_owned_link_skill || null
      );
      for (let p = 1; p <= 3; p++) {
        await upsertLinkSkills(
          ocid,
          p,
          link[`character_link_skill_preset_${p}`] || []
        );
      }
    }
    ...
    // 8b. Upsert Familiars
    const familiar = val(familiarData);
    if (familiar && familiar.familiar_info) {
      await upsertFamiliars(ocid, familiar.familiar_info);
    }

    // 8c. Upsert inner ability (lines per preset + active meta)
    const ability = val(abilityData);
    if (ability && ability.ability_info) {
      await upsertAbilities(ocid, ability);
    }
    ...
    if (pet) {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        if (name) {
          pets.push({
            pet_name: name,
            pet_nickname: pet[`pet_${i}_nickname`] || null,
            pet_icon: pet[`pet_${i}_icon`] || null,
            pet_description: pet[`pet_${i}_description`] || null,
            pet_type: pet[`pet_${i}_pet_type`] || null,
            pet_equipment_slot: `pet_${i}`,
            pet_total_option: pet[`pet_${i}_equipment`] || null,
            pet_auto_skill: pet[`pet_${i}_auto_skill`] || null,
            pet_skill: pet[`pet_${i}_skill`] || null,
            pet_date_expire: pet[`pet_${i}_date_expire`] || null,
            pet_appearance: pet[`pet_${i}_appearance`] || null,
            pet_appearance_icon: pet[`pet_${i}_appearance_icon`] || null,
          });
        }
      }
