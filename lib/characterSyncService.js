...
export async function syncCharacter(ocid) {
  try {
    ...
    if (champion && champion.union_champion) {
      // Fetch character images for each champion in parallel
      const championsWithImages = await Promise.all(
        champion.union_champion.map(async (c) => {
          if (!c.champion_name) return c;
          try {
            const champOcid = await getCharacterOcid(c.champion_name);
            const basicInfo = await getCharacterBasicInfo(champOcid);
            return {
              ...c,
              champion_ocid: champOcid || null,
