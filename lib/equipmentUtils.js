...
// Canonical display order for equipment slots — was byte-identical
// between components/equipment/EquipmentSection.js and
// components/compare/EquipmentCompareTab.js before being hoisted here.
...
// Slot key -> Chinese display label. Where the historical copies in
// EquipmentCompareTab.js drifted from the dashboard (components/
// EquipmentGrid.js / EquipmentList.js), the pre-existing dashboard
// labels win: '褲裙' (not '褲/裙'), '肩膀' (not '肩膀裝飾'), a single
// '戒指'/'墜飾'/'口袋' per slot (not numbered variants), and '機械心臟'
// (not '機器心臟').
...
// item_total_option stat key -> { label, isPercent }. Superset of
// components/equipment/EquipmentCardCompact.js's FALLBACK_STAT_LABELS,
// components/compare/EquipmentCompareTab.js's STAT_LABELS, and
// components/equipment/SpecialEquipmentPanel.js's STAT_FIELDS. On label
// conflicts (all_stat, boss_damage) EquipmentCardCompact's existing
// labels win.
export const EQUIPMENT_STAT_LABELS = {
  ...
  armor: { label: '防禦', isPercent: false },
};

// Slot-classification constants for the special-equipment slots (拼圖 /
// 圖騰 / 輔助特殊技能戒指 / 寶石) — moved verbatim from
// components/equipment/SpecialEquipmentPanel.js so this knowledge lives
// next to getEquipmentPosition.
...
export const analyzeScrolls = item => {
  ...
  const isGlove =
    item.item_equipment_part === '手套' || item.item_equipment_slot === '手套';
  ...
  if (category === 'weapon') {
    ...
    profiles = isGlove ? [...ARMOR_SCROLLS, ...GLOVE_TRACES] : ARMOR_SCROLLS;
  } else {
