...
export async function syncGuildMemberBasic(oguildId, characterName) {
  ...
  try {
    ...
    const [basicInfo, statsInfo] = await Promise.all([
      limiter.execute(() => getCharacterBasicInfo(ocid)),
      limiter.execute(() => getCharacterStats(ocid)).catch(() => null),
    ]);

    // Extract combat power from stats (same logic as characterSyncService)
    let combatPower = null;
    if (statsInfo?.final_stat) {
      const entry = statsInfo.final_stat.find(
        s => s.stat_name === '戰鬥力' || s.stat_name === '전투력'
      );
      if (entry) combatPower = entry.stat_value;
    }
