...
export const getCharacterFamiliar = async ocid => {
  try {
    const response = await apiClient.get(`/character/familiar?ocid=${ocid}`);
    return response.data;
  } catch (error) {
    throw new Error(`Failed to fetch familiar data: ${error.message}`);
  }
};

export const getCharacterAbility = async ocid => {
  try {
    const response = await apiClient.get(`/character/ability?ocid=${ocid}`);
    return response.data;
  } catch (error) {
